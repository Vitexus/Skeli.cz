...
public class EllipticPlayerServlet extends HttpServlet {
    ...
    @Override protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        out.println("<div class='player' id='el-player' style='position:relative;border:1px solid var(--panel-border);border-radius:12px;background:rgba(0,0,0,0.7);overflow:hidden;box-shadow:0 20px 50px rgba(0,0,0,.35);'>");
        out.println("  <div class='stage' style='position:relative;padding-top:28.125%;width:50%;min-width:320px;margin:0 auto;'><iframe id='el-frame' style='position:absolute;inset:0;width:100%;height:100%;border:0;' allow='autoplay; encrypted-media; picture-in-picture' allowfullscreen></iframe></div>");
        out.println("</div>");
        out.println("<div class='carousel' style='position:relative;height:190px;margin-top:12px;user-select:none;'>");
        ...
        out.println("const frame=document.getElementById('el-frame'); let current=0; let thumbs=[];\n"+
                "function play(i){ current=i; const v=vids[i]; frame.src='https://www.youtube.com/embed/'+v.yt+'?autoplay=1'; try{ localStorage.setItem('gp_id', v.yt); localStorage.setItem('gp_title', v.title);}catch(e){} if(window.playVideo) try{ window.playVideo(v.yt, v.title);}catch(e){} layout(); }\n"+
                ...
                "const track=document.getElementById('el-track'); function render(){ track.innerHTML=''; thumbs=vids.map((v,i)=>{ const el=document.createElement('div'); el.className='el-thumb'; el.style.cssText='position:absolute;top:50%;left:50%;width:240px;height:135px;margin:-67.5px 0 0 -120px;border-radius:12px;overflow:hidden;box-shadow:0 12px 30px rgba(0,0,0,.35);border:1px solid var(--panel-border);background:#000;transform-origin:center;transition:transform .32s ease,opacity .32s ease,filter .32s ease;cursor:pointer;'; el.innerHTML=`<img src='https://img.youtube.com/vi/${v.yt}/hqdefault.jpg' style='width:100%;height:100%;object-fit:cover;display:block;'><div style='position:absolute;left:8px;right:8px;bottom:6px;font-size:13px;background:linear-gradient(transparent, rgba(0,0,0,.7));padding:36px 6px 6px;color:#fff;text-shadow:0 1px 2px rgba(0,0,0,.5);border-radius:10px;'>${v.title}</div>`; el.addEventListener('click',()=>play(i)); track.appendChild(el); return el;}); layout(); if(vids.length) play(0); else frame.src='https://www.youtube.com/embed/dQw4w9WgXcQ'; } render();\n"+
                ...
                "document.getElementById('el-prev').onclick=()=>{ current=(current+vids.length-1)%vids.length; play(current); }; document.getElementById('el-next').onclick=()=>{ current=(current+1)%vids.length; play(current); };\n");
        out.println("})();");
        out.println("</script>");
    }
}
